...
public class ModelImpl implements Model {
    ...
    public boolean isLit(int r, int c) {
        if (r > lamps.length || c > lamps[0].length) {
            ...
            for (int row = 0; row < lamps.length; row++) {
                if (isLamp(row, c)) {
                    checker++;
                } else if (library.getPuzzle(puzzle).getCellType(row, c) == CellType.CLUE || library.getPuzzle(puzzle).getCellType(row, c) == CellType.WALL) {
                    if (row < r) {
                        checker = 0;
                    } else {
                        row = lamps.length;
            ...
            for (int col = 0; col < lamps[0].length; col++) {
                if (isLamp(r, col)) {
                    checker++;
                } else if (library.getPuzzle(puzzle).getCellType(r, col) == CellType.CLUE || library.getPuzzle(puzzle).getCellType(r, col) == CellType.WALL) {
                    if (col < c) {
                        checker = 0;
                    } else {
                        col = lamps[0].length;
    ...
    public void resetPuzzle() {
        lamps = new boolean[library.getPuzzle(puzzle).getHeight()][library.getPuzzle(puzzle).getWidth()];
        notifyObservers();
    }
